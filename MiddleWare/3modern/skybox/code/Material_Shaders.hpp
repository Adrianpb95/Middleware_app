...
namespace example
{
	...
	const std::string vertex_shader_code =
		...
		"    vec4  position = model_view_matrix * (vec4(vertex_coordinates.x, vertex_coordinates.y * coordinate_y, vertex_coordinates.z, 1.0));"
		""
		"	 vec3  aux = mesh_color;"
	...
	//Fragment shader de textura y luz
	...
	//Vertex shader de color plano y luz
	const std::string vertex_shader_code_color =
		...
		"	 light_diffuse_intensity = light.diffuse_intensity;"
		"	 light_color = light.color;"
		"	 front_color = vertex_color * mesh_color; "
		"    gl_Position = projection_matrix * position;"
		"}";

	
	//Fragment shader de color plano y luz
	const std::string fragment_shader_code_color =
		"#version 330\n"
		""
		"in  float light_intensity;"
		"in  float light_ambient_intensity;"
		...
		"void main()"
		"{"
		"    fragment_color = vec4(light_ambient_intensity * front_color, 1.0) + vec4(light_diffuse_intensity * light_color * light_intensity * front_color, 1.0);"
		"}";
